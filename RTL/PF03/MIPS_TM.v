module MIPS_TM (
    ...
    wire [31:0] idex_pc4_branch; // <--- CABLE PARA EL PC EN LA ETAPA EX
    ...
    BufferInstr ifid(
        .clk(clk),
        .reset(reset),
        .flush(pcsource | jump_sig), // <--- MODIFICADO: Vacía el buffer si hay salto
    ...
    DPTR dp(
        ...
        .pc4_in(ifid_pc4),            // <--- MODIFICADO: Manda el PC+4 al DPTR
        .result(out_result),
        .Branch_out(branch_sig), 
        .Zero_out(zero_sig), 
        .SignExt_out(sign_ext_val),
        .PC4_Branch_out(idex_pc4_branch), // <--- MODIFICADO: Recibe el PC alineado de la etapa EX
        .Jump_out(jump_sig),
    ...
    // MODIFICADO: Ahora suma el PC de la etapa correcta
    ...
    // Compuerta AND para el Branch (La lógica de inversión del BNE ya se hizo dentro del DPTR)
